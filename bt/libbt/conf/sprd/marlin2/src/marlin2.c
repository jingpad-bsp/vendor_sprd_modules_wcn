...
static pskey_config_t marlin2_pskey;

static const conf_entry_t marlin2_table[] = {
    {"pskey_cmd", 0, 0, 0, 0},

    CONF_ITEM_TABLE(g_dbg_source_sink_syn_test_data, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_sys_sleep_in_standby_supported, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_sys_sleep_master_supported, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_sys_sleep_slave_supported, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(default_ahb_clk, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(device_class, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(win_ext, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(g_aGainValue, 0, marlin2_pskey, 6),
    CONF_ITEM_TABLE(g_aPowerValue, 0, marlin2_pskey, 5),

    CONF_ITEM_TABLE(feature_set, 0, marlin2_pskey, 16),
    CONF_ITEM_TABLE(device_addr, 0, marlin2_pskey, 6),

    CONF_ITEM_TABLE(g_sys_sco_transmit_mode, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_sys_uart0_communication_supported, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(edr_tx_edr_delay, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(edr_rx_edr_delay, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(g_wbs_nv_117, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(is_wdg_supported, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(share_memo_rx_base_addr, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(g_wbs_nv_118, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_nbv_nv_117, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(share_memo_tx_packet_num_addr, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(share_memo_tx_data_base_addr, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(g_PrintLevel, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(share_memo_tx_block_length, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(share_memo_rx_block_length, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(share_memo_tx_water_mark, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(g_nbv_nv_118, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(uart_rx_watermark, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(uart_flow_control_thld, 0, marlin2_pskey, 1),

    CONF_ITEM_TABLE(comp_id, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(pcm_clk_divd, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(br_edr_diff_reserved, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_aBRChannelpwrvalue, 0, marlin2_pskey, 8),
    CONF_ITEM_TABLE(g_aEDRChannelpwrvalue, 0, marlin2_pskey, 8),
    CONF_ITEM_TABLE(g_aLEPowerControlFlag, 0, marlin2_pskey, 1),
    CONF_ITEM_TABLE(g_aLEChannelpwrvalue, 0, marlin2_pskey, 8),
    CONF_ITEM_TABLE(g_central_or_peripheral, 0, marlin2_pskey, 1),
    {0, 0, 0, 0, 0}
};
...
static void hw_core_cback(void *p_mem) {
    ...
    if (bt_vendor_cbacks) {
        ...
        ALOGI("%s error : bt_vendor_cbacks is null", __func__);
        return;
    }
    if (status) {
...
static void marlin2_pskey_dump(void *arg) {
    UNUSED(arg);
    pskey_config_t *p = &marlin2_pskey;
...
static int get_pskey_size(void) {
    int size = 0;
    conf_entry_t *p_entry = (conf_entry_t *)&marlin2_table;
...
static void marlin2_pskey_cback(void *p_mem) {
    HC_BT_HDR *p_evt_buf = (HC_BT_HDR *)p_mem;
...
static void marlin2_pskey_reload(pskey_config_t *pskey) {
    int ret, value;
...
static int marlin2_pskey_preload(void *arg) {
    HC_BT_HDR *p_buf = NULL;
    uint8_t *p, *src;
    int i = 0;
    ...
    if (p_buf) {
        ...
        *p = PSKEY_PREAMBLE_SIZE; /* parameter length */
        ...
        p++;
        marlin2_pskey_reload(&marlin2_pskey);

        UINT8_TO_STREAM(p, marlin2_pskey.g_dbg_source_sink_syn_test_data);
        UINT8_TO_STREAM(p, marlin2_pskey.g_sys_sleep_in_standby_supported);
        UINT8_TO_STREAM(p, marlin2_pskey.g_sys_sleep_master_supported);
        UINT8_TO_STREAM(p, marlin2_pskey.g_sys_sleep_slave_supported);

        UINT32_TO_STREAM(p, marlin2_pskey.default_ahb_clk);
        UINT32_TO_STREAM(p, marlin2_pskey.device_class);
        UINT32_TO_STREAM(p, marlin2_pskey.win_ext);

        for (i = 0; i < 6; i++) {
            UINT32_TO_STREAM(p, marlin2_pskey.g_aGainValue[i]);
        }
        for (i = 0; i < 5; i++) {
            UINT32_TO_STREAM(p, marlin2_pskey.g_aPowerValue[i]);
        }

        for (i = 0; i < 16; i++) {
            UINT8_TO_STREAM(p, marlin2_pskey.feature_set[i]);
        }
        for (i = 0; i < 6; i++) {
            UINT8_TO_STREAM(p, marlin2_pskey.device_addr[i]);
        }

        UINT8_TO_STREAM(p, marlin2_pskey.g_sys_sco_transmit_mode);
        UINT8_TO_STREAM(p, marlin2_pskey.g_sys_uart0_communication_supported);
        UINT8_TO_STREAM(p, marlin2_pskey.edr_tx_edr_delay);
        UINT8_TO_STREAM(p, marlin2_pskey.edr_rx_edr_delay);

        UINT16_TO_STREAM(p, marlin2_pskey.g_wbs_nv_117);

        UINT32_TO_STREAM(p, marlin2_pskey.is_wdg_supported);

        UINT32_TO_STREAM(p, marlin2_pskey.share_memo_rx_base_addr);
        UINT16_TO_STREAM(p, marlin2_pskey.g_wbs_nv_118);
        UINT16_TO_STREAM(p, marlin2_pskey.g_nbv_nv_117);

        UINT32_TO_STREAM(p, marlin2_pskey.share_memo_tx_packet_num_addr);
        UINT32_TO_STREAM(p, marlin2_pskey.share_memo_tx_data_base_addr);

        UINT32_TO_STREAM(p, marlin2_pskey.g_PrintLevel);

        UINT16_TO_STREAM(p, marlin2_pskey.share_memo_tx_block_length);
        UINT16_TO_STREAM(p, marlin2_pskey.share_memo_rx_block_length);
        UINT16_TO_STREAM(p, marlin2_pskey.share_memo_tx_water_mark);
        UINT16_TO_STREAM(p, marlin2_pskey.g_nbv_nv_118);

        UINT16_TO_STREAM(p, marlin2_pskey.uart_rx_watermark);
        UINT16_TO_STREAM(p, marlin2_pskey.uart_flow_control_thld);
        UINT32_TO_STREAM(p, marlin2_pskey.comp_id);
        UINT16_TO_STREAM(p, marlin2_pskey.pcm_clk_divd);

        UINT16_TO_STREAM(p, marlin2_pskey.br_edr_diff_reserved);
         for (i = 0; i < 8; i++) {
            UINT32_TO_STREAM(p, marlin2_pskey.g_aBRChannelpwrvalue[i]);
        }
        for (i = 0; i < 8; i++) {
            UINT32_TO_STREAM(p, marlin2_pskey.g_aEDRChannelpwrvalue[i]);
        }
        UINT32_TO_STREAM(p, marlin2_pskey.g_aLEPowerControlFlag);
        for (i = 0; i < 8; i++) {
            UINT16_TO_STREAM(p, marlin2_pskey.g_aLEChannelpwrvalue[i]);
        }
        UINT16_TO_STREAM(p, marlin2_pskey.g_central_or_peripheral);

        upio_set(UPIO_BT_WAKE, UPIO_ASSERT, 0);

        /* Send command via HC's xmit_cb API */
        bt_vendor_cbacks->xmit_cb(HCI_PSKEY, p_buf, marlin2_pskey_cback);
    } else {
        ALOGI("hw_pskey_send dont send pskey");
        if (bt_vendor_cbacks) {
            ALOGE("vendor lib hw_pskey_send aborted [no buffer]");
...
static void marlin2_epilog_process() {
    hw_core_enable(0);
}

static int marlin2_init(void) {
    int ret;
    char property[128] = {0};

    ALOGI("%s", __func__);
    memset(&marlin2_pskey, 0, sizeof(marlin2_pskey));
    ...
    vnd_load_configure(property, &marlin2_table[0]);

    set_mac_address(marlin2_pskey.device_addr);
    ...
    if (ret >= 0 && !strcmp(property, "open")) {
        ...
        ALOGI("SSP setting pskey from : 0x%02x to: 0x%02x", marlin2_pskey.feature_set[6],
              (marlin2_pskey.feature_set[6] & 0xF7));
        marlin2_pskey.feature_set[6] &= 0xF7;
    }
...
static bt_adapter_module_t marlin_module = {
    .name = "marlin2",
    .pskey = &marlin2_pskey,
    .tab = (conf_entry_t *)&marlin2_table,
    .init = marlin2_init,
    .pskey_preload = marlin2_pskey_preload,
    .epilog_process = marlin2_epilog_process,
    .pskey_dump = marlin2_pskey_dump,
    .get_conf_file = NULL
};
