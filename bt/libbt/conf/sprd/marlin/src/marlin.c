...
static const conf_entry_t marlin_table[] = {
    ...
    CONF_ITEM_TABLE(g_central_or_peripheral, 0, marlin_pskey, 1),
};
...
static int marlin_pskey_preload(void *arg) {
    ...
    if (p_buf) {
        ...
        *p = PSKEY_PREAMBLE_SIZE; /* parameter length */
        ...
        UINT16_TO_STREAM(p, marlin_pskey.g_central_or_peripheral);
...
#define MARLIN_HARDWARE_VERSION "persist.vendor.hw.marlin.version"
