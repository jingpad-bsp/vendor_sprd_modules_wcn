...
/*-----CMD ID:67-----------*/
int wlnpi_cmd_get_tx_status(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	int ret;
	unsigned long int val;
...
/*-----CMD ID:68-----------*/
int wlnpi_cmd_get_rx_status(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	int ret;
	unsigned long int val;
...
/*-----CMD ID:72-----------*/
int wlnpi_cmd_wmm_boost_mode(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
#define WMM_BOOST_ACTION_GET	0
#define WMM_BOOST_ACTION_SET	1

	int ret;
	unsigned char *action = (unsigned char *)(s_buf + 0);
	unsigned char *mode = (unsigned char *)(s_buf + 1);
	unsigned long int val;

	/* check input data */
	if (((argc != 1) && (argc != 2)) || (s_buf == NULL)) {
		ENG_LOG("ADL leaving %s(), argc = %d ERR\n", __func__, argc);
		return -1;
	}
	...
	ret = str2digit(argv[0], &val);
	if (ret < 0) {
		ENG_LOG("ADL leaving %s(), argv[0]=%s ERR\n", __func__, argv[0]);
		return -1;
	}
	...
	/* prepare mode value */
	if ((*action) == WMM_BOOST_ACTION_GET) {
		(*mode) = 0;
	} else {
		if (argc != 2) {
			ENG_LOG("ADL leaving %s(), argc = %d ERR\n",
		...
		if (ret < 0) {
			ENG_LOG("ADL leaving %s(), argv[1]=%s ERR\n",
					__func__, argv[1]);
			return -1;
		}
	...
	(*s_len) = 2 * sizeof(*s_buf);

	ENG_LOG("ADL leaving %s(), action = %d, mode = %d, length = %d",
			__func__, (*action), (*mode), (*s_len));

	return 0;

#undef WMM_BOOST_ACTION_GET
#undef WMM_BOOST_ACTION_SET
}

int wlnpi_show_wmm_boost_mode(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	if (r_len == 0) {
		...
		mode = *((unsigned char *)r_buf);
		printf("mode = %d\n", mode);
		ENG_LOG("ADL leaving %s(), mode = %d, return 0\n",
				__func__, mode);
	} else {
		printf("wrong r_len(=%d) value\n", r_len);
...
int wlnpi_show_get_qmu_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
	...
	for (i = 0; i < len; i++) {
		printf("%s = %d\n", type_name[i], *((unsigned short *)r_buf + i)); /*lint !e826 Note 826: suspicious pointer-to-pointer conversion (area too small)*/
	}
...
int wlnpi_cmd_set_ampdu_cnt(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
	...
	*((unsigned int *)s_buf) = strtol(argv[0], &err, 10);
	*((unsigned int *)(s_buf +1)) = strtol(argv[1], &err, 10);
	*((unsigned int *)(s_buf +2)) = strtol(argv[2], &err, 10);
	if (*s_buf < 0 || * s_buf > 2
			     ...
			     int r_len)
{
	...
	if (sizeof(struct tx_status_struct) != r_len)
	{
		printf("length mismatch: need=%u, r_len=%d\n",
				sizeof(struct tx_status_struct), r_len);
		ENG_LOG("ADL leaving %s(), r_buf=0x%p, r_len=%d need=%u ERROR, return -1",
			     ...
			     int r_len)
{
	...
	if (sizeof(struct rx_status_struct) != r_len)
	{
		printf("length mismatch: need=%u, r_len=%d\n",
...
#define STALUT_CAP_VHT_MCS9_BW20_BIT	BIT(13)
	...
	while ((offset + sz_wlnpi_sta_lut_t) <= r_len)
	{
		...
		slut_raw = (unsigned int *)(&(slut->mac_addr_l[0]));

		printf("[STALUT#%02d, Index:%02u]\r\n", i++, slut->lut_idx);

		printf("WORD0-WORD1:0x%08x-0x%08x\r\n", slut_raw[0], slut_raw[1]);
		...
		printf("WORD4-WORD7:0x%08x-0x%08x-0x%08x-0x%08x\r\n",
				slut_raw[4], slut_raw[5], slut_raw[6], slut_raw[7]);
		...
		printf("WORD8-WORD11:0x%08x-0x%08x-0x%08x-0x%08x\r\n",
				slut_raw[8], slut_raw[9], slut_raw[10], slut_raw[11]);
		...
		printf("WORD12-WORD15:0x%08x-0x%08x-0x%08x-0x%08x\r\n",
				slut_raw[12], slut_raw[13], slut_raw[14], slut_raw[15]);
...
int wlnpi_cmd_set_ini_reg_en(int argc, char **argv, signed char *s_buf, int *s_len)
{
	char *err;
...
int wlnpi_cmd_get_dpd_coeffi(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*((unsigned char*)s_buf) = val;
	...
	{
		ENG_LOG("lut_index error! val = %d", val);
		return -1;
	}
...
struct wlnpi_cmd_t g_cmd_table[] =
{
	...
	},
	{
		...
		.name  = "tx_status",
		.help  = "tx_status <ctxt_id>",
		.parse = wlnpi_cmd_get_tx_status,
	...
	},
	{
		...
		.name  = "rx_status",
		.help  = "rx_status <ctxt_id>",
		.parse = wlnpi_cmd_get_rx_status,
