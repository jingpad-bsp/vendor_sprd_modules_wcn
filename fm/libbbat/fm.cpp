...
extern DYMIC_WRITETOPC_FUNC g_func[WRITE_TO_MAX];
//------------------------------------------------------------------------------
...
int fmPlay( uint freq )
{
    ...
    if (status == NO_ERROR) {
		...
		LOGD("cannot write to AudioTrack: status = %d\n", status);
	}
	atrk.stop();
#endif
...
int fmCheckStatus(uchar *fm_status )
{
    ...
    if( ret < 0 ) {
        LOGD("ioctl error: %s  ret %d  fm_status %d\n", strerror(errno), ret, fm_status);
        return ret;
    }
...
int Radio_Open( uint freq )
{
    ...
    if (fd < 0) {
        LOGD("open err! fd = %d\n", fd);
        LOGD("Radio_Open fail, errno = %d, %s !!!\n", errno, strerror(errno));
        return -1;
    }

    if ( freq < 8750 || freq > 10800 ) {
        LOGD("invalid freq! freq = %d\n", freq);
        return -1;
    } else {
        parm.freq = freq;
    }
    ...
    if (g_func[WRITE_TO_NPISO_AT] != NULL) {
        ...
        snprintf(req_open, sizeof(req_open),  "%s", "AT+HEADSETCHECK");
        LOGD("FT: send_at_test req_fm: req_open=%s",req_open);
...
int Radio_Play( uint freq )
{
	...
	if (g_func[WRITE_TO_NPISO_AT] != NULL) {
        char req_fm[256] = {0};
        char req_headset[256] = {0};
        LOGD("%s enter ", __FUNCTION__);

        snprintf(req_fm, sizeof(req_fm),  "%s=%d,%d", "AT+AUDIOFM",1,8);
        g_func[WRITE_TO_NPISO_AT](req_fm, sizeof(req_fm));

        usleep(100000);
        snprintf(req_headset, sizeof(req_headset),  "%s=%s=%d;%s=%d", "AT+AUDIOPIPE","routing",4,"set_fm_speaker",0);
        LOGD("FT: send_at_test req_fm: req_fm=%s,req_headset=%s",req_fm,req_headset);
        g_func[WRITE_TO_NPISO_AT](req_headset, sizeof(req_headset));
    }
...
int Radio_Close( void )
{
	...
	if (g_func[WRITE_TO_NPISO_AT] != NULL) {
        char req_close[256] = {0};
        LOGD("%s enter xiaopeng9", __FUNCTION__);
        snprintf(req_close, sizeof(req_close),  "%s=%d", "AT+AUDIOFM",0);
        LOGD("FT: send_at_test: req_close=%s",req_close);
        g_func[WRITE_TO_NPISO_AT](req_close, sizeof(req_close));
    }

    ret = ioctl(fd, FM_IOCTL_POWERDOWN, &type);
    if (ret) {
        LOGD("power down err! ret = %d\n", ret);
        return ret;
    }

    ret = close(fd);
    if (ret) {
        LOGD("close err! ret = %d\n", ret);
        return ret;
    }
