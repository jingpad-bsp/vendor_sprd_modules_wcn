...
int start_fm_test(char * buf,int len,char *rsp,int rsplen)
{
	unsigned int i;
	int ret = 0;
	...
	return translate_packet(rsp,tmprsp,rsplen);
}

void register_this_module_ext(struct eng_callback * reg,int *num)
{
	ALOGD("%s : for FM EUT ", __func__);
	int moudles_num = 0;

	reg->type = 0x41; //main cmd
	reg->subtype = 0x0; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	reg->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;

	(reg+1)->type = 0x41; //main cmd
	(reg+1)->subtype = 0x1; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	(reg+1)->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;

	(reg+2)->type = 0x41; //main cmd
	(reg+2)->subtype = 0x2; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	(reg+2)->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;

	(reg+3)->type = 0x41; //main cmd
	(reg+3)->subtype = 0x3; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	(reg+3)->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;

	(reg+4)->type = 0x41; //main cmd
	(reg+4)->subtype = 0x4; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	(reg+4)->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;

	(reg+5)->type = 0x41; //main cmd
	(reg+5)->subtype = 0x5; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	(reg+5)->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;

	(reg+6)->type = 0x41; //main cmd
	(reg+6)->subtype = 0x6; //sub cmd
	//reg->diag_ap_cmd = 0x1; // fixnv query
	(reg+6)->eng_diag_func = start_fm_test; // rsp function ptr
	moudles_num++;
